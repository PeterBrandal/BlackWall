...
function expiryContext(daysLeft: number): { text: string; color: string } {
  if (daysLeft < 0)
    return {
      text: 'Certificate has expired — site may show security warnings.',
      color: 'text-crimson',
    }
  if (daysLeft < 14)
    return {
      text: `Expires in ${daysLeft} days — critical, renewal needed immediately.`,
      color: 'text-crimson/80',
    }
  if (daysLeft < 30)
    return { text: `Expires in ${daysLeft} days — renew soon.`, color: 'text-yellow-400/70' }
...
function issuerContext(issuer: string): string | null {
  if (
    ...
    issuer.toLowerCase().includes('lets encrypt')
  )
...
export function SslCard({ state }: Props) {
  ...
  return (
    <ProbeCard title="SSL / TLS CERTIFICATE" status={state.status} color="text-orange-400/50">
      ...
      {expCtx && <p className={`mb-2 font-mono text-xs ${expCtx.color}`}>{expCtx.text}</p>}
      {issuerCtx && <p className="mb-3 font-mono text-xs text-white/25">{issuerCtx}</p>}

      {sans.length > 0 && (
        <div className="border-t border-white/5 pt-2">
          <p className="mb-1 font-mono text-xs text-white/20 tracking-widest">ALT NAMES</p>
          {sans.map((san) => (
            <p key={san} className="font-mono text-xs text-orange-400/50 truncate">
              {san}
            </p>
          ))}
        </div>
