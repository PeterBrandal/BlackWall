...
export function RedirectCard({ state }: Props) {
  ...
  const cdn = state.lines
    .find((l) => l.includes('CDN          :'))
    ?.split(':')
    .slice(1)
    .join(':')
    .trim()
  ...
  return (
    <ProbeCard title="REDIRECT CHAIN" status={state.status} color="text-violet-400/50">
      {ctx && <p className="mb-3 font-mono text-xs text-white/30">{ctx}</p>}
      ...
      {cdn && (
        <div className="border-t border-white/5 pt-2">
          ...
          <p className="mt-1 font-mono text-xs text-white/25">
            Traffic passes through {cdn} before reaching the origin server — improves speed and
