  ...
  return (
    <ProbeCard title="GDPR COMPLIANCE ANALYSIS" status={state.status} color={cardColor}>
      ...
      <div className="mb-4 flex items-center gap-3">
        <span className={`font-mono text-xs px-2 py-1 border tracking-widest ${
          g.violation
          ...
          {g.violation ? "⚠ LIKELY VIOLATION" : g.trackers.length > 0 ? "⚠ REVIEW REQUIRED" : "✓ NO ISSUES FOUND"}
        </span>
        {g.dpa && (
          <span className="font-mono text-xs text-white/25">{g.dpa}</span>
        )}
      </div>
      ...
      <div className="mb-3">
        ...
        {g.cmps.length > 0 ? (
          g.cmps.map((cmp) => (
            <span key={cmp} className="inline-block mr-2 font-mono text-xs border border-green-400/30 text-green-400/70 px-2 py-0.5">
              ✓ {cmp}
            </span>
          ))
        ) : (
      ...
      {g.schrems2.length > 0 && (
        <div className="mt-3 border-t border-white/5 pt-3">
          ...
          <p className="font-mono text-xs text-orange-400/60 leading-relaxed">
            {g.schrems2.join(", ")} route data to US-based processors.
            Without verifiable Standard Contractual Clauses, this may violate GDPR Ch. V.
          </p>
        </div>
      )}
      ...
      <p className="mt-4 font-mono text-xs text-white/15 leading-relaxed">
        ⚠ Static HTML analysis only. JavaScript-loaded trackers and runtime consent flow require browser-level verification.
      </p>
    </ProbeCard>
  )
}
