...
function domainAgeContext(created: string): { text: string; color: string } | null {
  ...
  if (days < 90)
    return {
      text: 'Registered less than 3 months ago — treat with caution.',
      color: 'text-crimson/70',
    }
  if (days < 365)
    return {
      text: 'Registered less than a year ago — relatively new domain.',
      color: 'text-yellow-400/60',
    }
  if (days > 365 * 10)
    return {
      text: 'Established domain — registered over 10 years ago.',
...
export function WhoisCard({ state }: Props) {
  ...
  return (
    <ProbeCard title="WHOIS // DOMAIN REGISTRY" status={state.status} color="text-yellow-400/50">
      ...
      {ageCtx && <p className={`mb-3 font-mono text-xs ${ageCtx.color}`}>{ageCtx.text}</p>}
      ...
      {w.nameservers.length > 0 && (
        <div className="border-t border-white/5 pt-2">
          ...
          {w.nameservers.map((ns) => (
            <p key={ns} className="font-mono text-xs text-yellow-400/60">
              {ns}
            </p>
          ))}
        </div>
