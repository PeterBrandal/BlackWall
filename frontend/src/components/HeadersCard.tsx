...
export function HeadersCard({ state }: Props) {
  const server  = state.lines.find((l) => l.includes("Server  :"))?.split(":").slice(1).join(":").trim()
  const powered = state.lines.find((l) => l.includes("Powered :"))?.split(":").slice(1).join(":").trim()
    ...
    <ProbeCard title="HTTP SECURITY HEADERS" status={state.status} color="text-pink-400/50">
      ...
      {(server || powered) && (
        <div className="mb-3 space-y-1 font-mono text-xs">
          {server  && <div className="flex justify-between gap-4"><span className="text-white/25">SERVER</span><span className="text-pink-400/70">{server}</span></div>}
          {powered && powered !== "—" && <div className="flex justify-between gap-4"><span className="text-white/25">POWERED BY</span><span className="text-pink-400/70">{powered}</span></div>}
        </div>
      )}
